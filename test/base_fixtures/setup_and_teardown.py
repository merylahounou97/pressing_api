...
def setup_and_teardown(generate_user_data):
    ...
    _customers = [generate_user_data(role=UserRole.CUSTOMER) for _ in range(2)] + [generate_user_data(role=UserRole.CUSTOMER,email_verified=True,phone_number_verified=True) ]
    _secretaries = [generate_user_data(role=UserRole.SECRETARY) for _ in range(2)]
    _admins = [generate_user_data(role=UserRole.ADMIN, phone_number="+33666495244"), generate_user_data(role=UserRole.ADMIN, email="aiounouu@gmail.com")]
