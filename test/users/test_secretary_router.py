...
class  TestSecretaryRouter():
    ...
    def test_create_secretary(self,generate_user_data,get_access_token,create_user):
        """Test the creation of a secratary"""
        ...
        response = client.post(f"{BASE_URL}/member", json=user,headers={"Authorization": f"Bearer {access_token}"})
        ...
        response = client.post(f"{BASE_URL}/member", json=user,headers={"Authorization": f"Bearer {admin_token}"})
