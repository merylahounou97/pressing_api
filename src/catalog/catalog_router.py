...
from src.catalog.catalog_schemas import ArticleCreateInputSchema, ArticleEditInputSchema, ArticleOutputSchema
...
@router.patch("/{article_id}", dependencies=[Depends(get_user_online_dep(
    roles=[UserRole.ADMIN, UserRole.SECRETARY]))])
async def edit_article(article_id: str, article_input: ArticleEditInputSchema,
                       catalog_service=Depends(CatalogService))-> ArticleOutputSchema | None:
    """Edit an article
