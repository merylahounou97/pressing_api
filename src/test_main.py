...
def mock_authenticate_user(mocker):
    return mocker.patch("src.sql.login.login_service.authenticate_user",
                    return_value =  {
                        "id":"test_id",
                        "email":"a@gmail.com",
                        "last_name":"last",
...
def test_login_valid(mock_authenticate_user):
    ...
    response = client.post("/login", 
                            json = {"identifier": "a@gmail.com", "password": "password123"})
    ...
    # Envoyer une requête POST avec des identifiants valides
    response = client.post("/login", 
                            json = {"identifier": "+33745346489", "password": "password123"})
