...
from src.customer import customer_schema
...
async def  create_customers(customer: Customer_create_input,redirect_url: str , db: Session = Depends(get_db)):
...
def verify_code(verification: Customer_verify_code , db: Session = Depends(get_db)):
    return customer_service.verify_code(verification=verification,db=db,strategy="phone_number")

@router.post("/verify_email")
def verify_email(verification: Customer_verify_code , db: Session = Depends(get_db)):
    return customer_service.verify_code(verification=verification,db=db,strategy="email")

@router.post("/generate_new_email_validation_code")
def generate_new_email_validation_code (create_new_validation_code_input: customer_schema.Customer_generate_new_validation_code_input):
    return customer_service.generate_new_validation_code(create_new_validation_code_input)