...
from src.order.order_schemas import OrderCreateInputSchema, OrderCreateOutputSchema, OrderEditInputSchema
...
@router.patch("/{order_id}", response_model=OrderCreateOutputSchema, 
              dependencies=[Depends(get_user_online_dep(roles=[UserRole.ADMIN, UserRole.SECRETARY]))])
...
async def get_all_orders(order_service=Depends(OrderService), user=Depends(get_user_online_dep(roles=[UserRole.ADMIN, UserRole.SECRETARY]))):
...
async def cancel_order(order_id: str, order_service=Depends(OrderService), user=Depends(get_user_online_dep(roles=[UserRole.ADMIN, UserRole.SECRETARY]))):
    """Cancel an order."""
    return order_service.cancel_order(order_id)

@router.get("/history/{customer_id}", response_model=list[OrderCreateOutputSchema])
async def get_order_history(customer_id: str, order_service=Depends(OrderService), user=Depends(get_user_online_dep())):
    """Get order history."""
    if not customer_id:
        raise HTTPException(status_code=400, detail=ErrorMessages.USER_ID_NOT_PROVIDED)
