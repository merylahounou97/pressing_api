...
from src.order.order_schemas import OrderCreateInputSchema, OrderCreateOutputSchema, OrderEditInputSchema, OrderDetailSchema
...
class OrderService:
    ...
    def get_order_by_id(self, order_id: int) -> OrderCreateOutputSchema:
        ...
        order_details = self.db.query(OrderDetailsModel).filter(OrderDetailsModel.order_id == order_id).all()
        order_dict = order.__dict__.copy()
        order_dict["order_details"] = [OrderDetailSchema(**detail.__dict__) for detail in order_details]
            ...
            for key, value in order_edit_input.model_dump(exclude_unset=True).items():
                ...
                if key == "order_details":
                    for detail in value:
                        ...
                        order_detail = self.db.query(OrderDetailsModel)\
                        .filter(OrderDetailsModel.order_id == order_id, OrderDetailsModel.article_id == detail["article_id"]).first()
                        if order_detail:
    ...
    def get_all_orders(self):
        ...
        results = self.db.query(OrderModel).join(OrderDetailsModel, OrderModel.id == OrderDetailsModel.order_id).add_entity(OrderDetailsModel).all()
    ...
    def match_orders_to_order_details(self, results):
        ...
        for  order in orders:
            ...
            order_details_fitered = filter(lambda x: x[1].order_id == order["id"], results)
    ...
    def get_order_history(self, customer_id: str):
        ...
        return self.match_orders_to_order_details(orders)
